function [ h ] = shadedBarYlog( x, m, stds, color,line)
...
y= [m-stds fliplr(m+stds)];y(y<eps)=eps;%remove negative
h.fill = fill(x1,log(y),color, 'linestyle', 'none');
% Choose a number between 0 (invisible) and 1 (opaque) for facealpha.  
if line
    set(h.fill,'facealpha',.1)
    hold on;
    h.plot= plot(x, log(m), 'color', color, 'linewidth', 2);
end
end
